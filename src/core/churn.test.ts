...
describe("evaluateChurn", () => {
  ...
  it("no leave when at minAgents", () => {
    ...
    const decision = evaluateChurn(active, [], { minAgents: 3, maxAgents: 5 }, phrases);
    expect(decision.leave).toBeUndefined();
  });

  it("no join when at maxAgents", () => {
    const active = [makeAgent("A"), makeAgent("B"), makeAgent("C"), makeAgent("D"), makeAgent("E")];
    const benched = [makeAgent("F")];
    randomSpy.mockReturnValue(0); // force join to trigger if possible
    const decision = evaluateChurn(active, benched, { minAgents: 3, maxAgents: 5 }, phrases);
    ...
    const activeAllPriority = active.map((a, i) => makeAgent(a.personality.name, { priority: i }));
    const d2 = evaluateChurn(activeAllPriority, benched, { minAgents: 3, maxAgents: 5 }, phrases);
  ...
  it("priority agents never evicted", () => {
    ...
    const decision = evaluateChurn(active, [], { minAgents: 2, maxAgents: 5 }, phrases);
    if (decision.leave) {
  ...
  it("leave + join can happen in same turn", () => {
    const active = [makeAgent("A", { chattiness: 0 }), makeAgent("B"), makeAgent("C"), makeAgent("D")];
  ...
  it("agent who just left excluded from join pool", () => {
    ...
    if (decision.leave && decision.join) {
      expect(decision.join.agent.personality.name).not.toBe(decision.leave.agent.personality.name);
    }
  ...
  it("empty benched pool means no join", () => {
    const active = [makeAgent("A"), makeAgent("B"), makeAgent("C"), makeAgent("D")];
    randomSpy.mockReturnValue(0);
    const decision = evaluateChurn(active, [], { minAgents: 3, maxAgents: 5 }, phrases);
    expect(decision.join).toBeUndefined();
  });
  ...
  it("all-priority active pool means no leave", () => {
    ...
    randomSpy.mockReturnValue(0);
    const decision = evaluateChurn(active, [makeAgent("E")], { minAgents: 3, maxAgents: 5 }, phrases);
    expect(decision.leave).toBeUndefined();
  });
});
